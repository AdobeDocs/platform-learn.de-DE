---
user-guide-title: Migrieren von Target von at.js 2.x zum Web SDK
breadcrumb-title: Migrieren von Target von at.js 2.x zum Web SDK
user-guide-description: Ein praktisches Tutorial für Target-Kunden, in dem beschrieben wird, wie Sie erfolgreich von einer at.js- zu einer Web SDK-Implementierung migrieren können.
feature-set: Target,Experience Platform
solution: Experience Platform,Data Collection,Target
feature: Implement Client-side,APIs/SDKs,at.js,AEP Web SDK, Web SDK,Implementation
role: Developer
level: Intermediate,Experienced
doc-type: tutorial
recommendations: catalog,noDisplay
hide: true
kt: 11159
source-git-commit: 04fb22faf26eb69b916570e3990cc0c760cb7d4d
workflow-type: tm+mt
source-wordcount: '87'
ht-degree: 5%

---


# Migrieren einer Target at.js-Implementierung zum Web SDK {#migrate-target-to-websdk}

+ [Einführung in Tutorials](introduction.md)

+ [Übersicht über die Migration](migration-overview.md)

+ [Migration planen](plan-migration.md)

+ [Vergleichen von at.js mit dem Platform Web SDK](detailed-comparison.md)

+ [Ersteinrichtung durchführen](initial-setup.md)

+ [Ersetzen der Bibliothek und Vorab-Ausblendung](replace-library.md)

+ [VEC-basierte Aktivitäten rendern](render-vec-activities.md)

+ [Formularbasierte Aktivitäten rendern](render-form-based-activities.md)

+ [Versandparameter](send-parameters.md)

+ [Konversionsereignisse verfolgen](track-events.md)

+ [Domänenübergreifende Unterstützung aktivieren](cross-domain.md)

+ [Zielgruppen und Profilskripte aktualisieren](update-audiences.md)

+ [Validieren der -Implementierung](validate.md)

+ [Debug Target](debugging.md)
