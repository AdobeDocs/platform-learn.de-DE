...
| Verfolgen benutzerspezifischer Ereignisse | Unterstützt mit `trackEvent()` und `sendNotifications()` Funktionen. Diese Funktionen sind spezifisch für Target und wirken sich nicht auf Adobe Analytics-Metriken aus. | Alle Daten aus dem Platform Web SDK `sendEvent` -Aufrufe an Target weitergeleitet werden. Zusätzliche Daten, die speziell für Target benötigt werden, sollten in die `sendEvent` mit einem eventType von `decisioning.propositionDisplay` oder `decisioning.propositionInteract` , um sicherzustellen, dass Adobe Analytics-Metriken nicht betroffen sind. |
...
| `deviceIdLifetime` | Nicht unterstützt. Wenn `targetMigrationEnabled` auf `true` mit dem `configure` -Befehl, `mbox` -Cookie gesetzt wird, wobei die Gerätelebensdauer auf 2 Jahre festgelegt ist. Dieser Wert kann nicht konfiguriert werden. |
...
| `optinEnabled` und `optoutEnabled` | Siehe Platform Web SDK . [Datenschutzoptionen](https://experienceleague.adobe.com/docs/experience-platform/edge/consent/supporting-consent.html). Die `defaultConsent` -Option gilt für alle Adobe-Lösungen, die das Platform Web SDK unterstützt. |
