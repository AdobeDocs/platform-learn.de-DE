...
>[!NOTE]
>
>In diesem Tutorial wird die [Luma-Website](https://newluma.enablementadobe.com){target=_blank} verwendet, die über eine asynchrone Implementierung von Tags und Flimmerschutz verfügt. Dieser Abschnitt dient als Referenz, um zu verstehen, wie die Flimmerreduzierung mit Platform Web SDK funktioniert.
...
1. Klicken Sie auf **[!UICONTROL Schaltfläche (Stil zum Vorab-Ausblenden bearbeiten]**.
   ...
   ![Target-pre-hiding-Snippet für synchrone Implementierungen](assets/target-flicker-sync.png)
...
1. `all pages - adobeDataLayer push - send event - 50` öffnen
...
1. Erstellen Sie eine Experience Targeting-(XT)-Aktivität auf der Luma-Homepage für die Aktivitäts-URL und ggf. in Ihrem Target-Arbeitsbereich:
...
1. Aktualisieren Sie den Aktivitätsnamen und klicken Sie dann auf **[!UICONTROL Weiter]**.
...
1. Fügen Sie eine Bedingung hinzu, um die Regel auf die Luma-Homepage zu beschränken (Pfad ohne Abfragezeichenfolge `/` oder `/index.html`).
...
1. Fügen Sie den folgenden Code in das Feld Inhalt ein, um eine Änderung an der call-to-action in Hero vorzunehmen:
...
Es gibt für Target nützliche Parameter, die nicht vom XDM-Objekt zugeordnet sind. Zu diesen speziellen Zielgruppen-Parametern gehören:
...
* [Reservierte Parameter für Recommendations](https://experienceleague.adobe.com/de/docs/target/using/recommendations/plan-implement#pass-behavioral)
...
1. Öffnen Sie die Tag-Regel `product detail pages - adobeDataLayer push - set product details variables - 20`
...
1. Wählen Sie das **[!UICONTROL target]**-Objekt aus
1. `entity.id` als `Ecommerce Product Id` Datenelement festlegen
1. `entity.name` als `Ecommerce Product Name` Datenelement festlegen
1. `user.categoryId` als `Ecommerce Product Category` Datenelement festlegen
1. Wählen **[!UICONTROL Änderungen beibehalten]**
1. Wählen Sie **[!UICONTROL Speichern]**
...
1. `all pages - adobeDataLayer push - send event - 50` öffnen
1. `Adobe Experience Platform Web SDK - Send event` auswählen
1. Fügen Sie das `Data Variable` Datenelement zum Datenfeld hinzu, falls es noch nicht hinzugefügt wurde
...
Auf der Luma-Site wird die Datenschicht schnell geladen und ist vollständig definiert, bevor der Aufruf „Ereignis senden“ ausgelöst wird. Auf diese Weise können wir mit einem einzigen Aufruf personalisierte Inhalte abrufen (z. B. aus Adobe Target) und Analysedaten senden (z. B. an Adobe Analytics).
...
Der zweite Aufruf ist auch für das Reporting aller Ansichten personalisierter Inhalte verantwortlich und ersetzt alle `decisioning.propositionDisplay` Aufrufe.
...
1. Wählen **[!UICONTROL Änderungen beibehalten]**
1. **[!UICONTROL Speichern]** der Regel

   ![send_decision_request_alone](assets/target-decision-request.png)

So erstellen Sie die Analytics-On-Bottom-Anfrage:

1. Erstellen Sie eine neue Regel mit dem Namen `all pages - window loaded - send event - 50`
1. Fügen Sie der Regel ein Ereignis hinzu. Verwenden der **Core**-Erweiterung und des Ereignistyps **[!UICONTROL Window Loaded]**
...
1. Dadurch wird das Kontrollkästchen **[!UICONTROL Gerenderte Vorschläge einbeziehen]** aktiviert, damit die Benachrichtigungen der Entscheidungsanfrage in der Warteschlange gesendet werden.
1. Wählen **[!UICONTROL Änderungen beibehalten]**
1. **[!UICONTROL Speichern]** der Regel
1. Bibliothek aktualisieren und neu erstellen

![send_decision_request_alone](assets/target-aa-request-guided.png)

>[!TIP]
>
>Wenn auf das Ereignis, mit dem Sie einen Entscheidungsvorschlag abrufen, kein Adobe Analytics-Ereignis folgt, lassen Sie &quot;**[!UICONTROL verwenden“]**. Sie müssen alle Optionen manuell auswählen, aber dadurch wird die Option &quot;**[!UICONTROL gerenderte Vorschläge einschließen]** zusammen mit Ihrer Abrufanfrage freigegeben.
...
1. Navigieren Sie zur [Demo-Site von Luma](https://newluma.enablementadobe.com)
